...
def insert_order_tracking(order_id, status,session_id: str,my_order: str):
    cursor = cnx.cursor()

    formatted_date = now.strftime("%Y-%m-%d %H:%M:%S")
    # Inserting the record into the order_tracking table
    insert_query = "INSERT INTO order_tracking (order_id, status,session_id,my_order,order_date) VALUES (%s,%s,%s,%s,%s)"

    cursor.execute(insert_query, (order_id, status,session_id,my_order,formatted_date))
...
def get_past_orders(session_id):
    # Create a cursor object to execute SQL queries
    cursor = cnx.cursor()

    query = """
    SELECT order_id, my_order, order_status
    FROM order_tracking WHERE session_id = %s
    ...
    # Print the last 3 orders in a well-arranged way
    for result in results:
        order_id, my_order, order_status = result
        res = f"Order ID: {order_id}, My Order: {my_order}, Order Status: {order_status}. \n"
